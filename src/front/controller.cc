...
void s21::Controller::on_equalButton_clicked() {
  ...
  try {
    ...
    msg.setText(err);
    msg.exec();
    if (!msg.isActiveWindow()) {
      ui->mainLineEdit->clear();
      ui->mainLineEdit->setStyleSheet("QLineEdit{background: none;}");
      model_.CleanAllStacksLinesAndVectors();
    }
  }
}
...
void s21::Controller::on_printGraphButton_clicked() {
  try {
    ...
    model_.SetExpression(ui->mainLineEdit->text().toStdString());
    model_.SetX(ui->xLineEdit->text().toDouble());
    model_.CalculateGraphic(
        ui->xMinLine->text().toDouble(), ui->xMaxLine->text().toDouble(),
        ui->yMinLine->text().toDouble(), ui->yMaxLine->text().toDouble());
    xCoordinateVec_ = graph_->GetAddressOfxCoordinatesVec();
    yCoordinateVec_ = graph_->GetAddressOfyCoordinatesVec();
    *xCoordinateVec_ =
        QVector<double>(model_.GetXPoint()->begin(), model_.GetXPoint()->end());
    *yCoordinateVec_ =
        QVector<double>(model_.GetYPoint()->begin(), model_.GetYPoint()->end());
    ...
    msg.exec();
    model_.CleanAllStacksLinesAndVectors();
    if (!msg.isActiveWindow()) graph_->~Graph();
  }
}
